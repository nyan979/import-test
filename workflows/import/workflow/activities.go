...
func (a *Activities) GetPresignedUrl(config UploadTypeConfiguration) (string, error) {
	url, err := a.MinioClient.PresignedPutObject(context.Background(),
		os.Getenv("MINIO_BUCKET_NAME"), string(config[0].FileKey),
		time.Duration(config[0].UploadExpiryDurationInSec)*time.Second)
	if err != nil {
		log.Fatalln(err)
		return "", err
	}
	url.Host = os.Getenv("MINIO_DOMAIN_NAME")
	return url.String(), nil
}

func (a *Activities) PresignedUpload(bucket string, expire time.Duration, objectName string) (string, error) {
	url, err := a.MinioClient.PresignedPutObject(context.Background(), bucket, objectName, expire)
	if err != nil {
		log.Fatalln(err)
		return "", err
	}
...
func (a *Activities) PresignedDownload(bucket string, expire time.Duration, objectName string) (string, error) {
	url, err := a.MinioClient.PresignedGetObject(context.Background(), bucket, objectName, expire, url.Values{})
	if err != nil {
		log.Fatalln(err)
		return "", err
	}
...
func (a *Activities) ParseCSV(filekey string) ([]string, error) {
	object, err := a.MinioClient.GetObject(context.Background(),
		os.Getenv("MINIO_BUCKET_NAME"), filekey, minio.GetObjectOptions{})
	if err != nil {
		return nil, err
	}
	var lines []string
